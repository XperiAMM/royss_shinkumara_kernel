/*
 *  drivers/cpufreq/cpufreq_rodemvolt.c
 *
...
#define RODEMVOLT_MAJOR_VERSION	4
#define RODEMVOLT_MINOR_VERSION	2
...
#define DBS_INPUT_EVENT_MIN_FREQ		(1008000)
#define DBS_SYNC_FREQ				(800000)
#define DBS_OPTIMAL_FREQ			(1152000)
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_RODEMVOLT
static
#endif
struct cpufreq_governor cpufreq_gov_rodemvolt = {
       .name                   = "rodemvolt",
...
static int rodemvolt_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void rodemvolt_powersave_bias_init_cpu(int cpu)
{
...
static void rodemvolt_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		rodemvolt_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_rodemvolt Governor Tunables */
...
skip_this_cpu:
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using rodemvolt, cancel dbs timer */
				mutex_lock(&dbs_info->timer_mutex);
				dbs_timer_exit(dbs_info);

				rodemvolt_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of rodemvolt, waiting for disk IO is an
...
static void dbs_chown(void)
{
	...
	ret =
	sys_chown("/sys/devices/system/cpu/cpufreq/rodemvolt/sampling_rate",
		low2highuid(AID_SYSTEM), low2highgid(0));
	ret =
	sys_chown("/sys/devices/system/cpu/cpufreq/rodemvolt/boostpulse",
		low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	policy = this_dbs_info->cur_policy;
	if (!policy) {
		/* CPU not using rodemvolt governor */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		rodemvolt_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!rodemvolt_powersave_bias_setspeed(
...
#ifdef CONFIG_EARLYSUSPEND
static void cpufreq_rodemvolt_early_suspend(struct early_suspend *h)
{
	mutex_lock(&dbs_mutex);
...
static void cpufreq_rodemvolt_late_resume(struct early_suspend *h)
{
	mutex_lock(&dbs_mutex);
...
static struct early_suspend cpufreq_rodemvolt_early_suspend_info = {
	.suspend = cpufreq_rodemvolt_early_suspend,
	.resume = cpufreq_rodemvolt_late_resume,
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&cpufreq_rodemvolt_early_suspend_info);
#endif
	return cpufreq_register_governor(&cpufreq_gov_rodemvolt);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_rodemvolt);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
			&per_cpu(id_cpu_dbs_info, i);
...
#ifdef CONFIG_EARLYSUSPEND
	unregister_early_suspend(&cpufreq_rodemvolt_early_suspend_info);
#endif
...
MODULE_DESCRIPTION("'cpufreq_rodemvolt' - An intelligent dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_RODEMVOLT
