/*
 *  drivers/cpufreq/cpufreq_shinkumara.c
 *
 *  Copyright (C)  2014 Khristian 2014
 *             <khristian_chandra@yahoo.com>.
 *                      Shinkumara <khristian_chandra@yahoo.com>
 *
 *                 2014 Minor Edits by Khristian <khristian_chandra@yahoo.com>
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SHINKUMARA
static
#endif
struct cpufreq_governor cpufreq_gov_shinkumara = {
		.name                   = "shinkumara",
...
static struct workqueue_struct	*kshinkumara_wq;
...
// shinkumara suspend mods 
static unsigned int suspended = 0;
static void shinkumara_suspend(int suspend)
{
	...
	if (!suspend) { // resume at max speed:
		...
		__cpufreq_driver_target(dbs_info->cur_policy, dbs_info->cur_policy->max,
				CPUFREQ_RELATION_L);
		pr_info("[shinkumara] shinkumara awake at %d\n", dbs_info->cur_policy->cur);
	} else {
		suspended = 1;
		...
		pr_info("[shinkumara] shinkumara suspended at %d\n", dbs_info->cur_policy->cur);
	}
}

static void shinkumara_early_suspend(struct early_suspend *handler) {
	shinkumara_suspend(1);
}

static void shinkumara_late_resume(struct early_suspend *handler) {
	shinkumara_suspend(0);
}

static struct early_suspend shinkumara_power_suspend = {
		.suspend = shinkumara_early_suspend,
		.resume = shinkumara_late_resume,
...
static void shinkumara_powersave_bias_init_cpu(int cpu)
{
...
static void shinkumara_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		shinkumara_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_shinkumara Governor Tunables */
		...
		const char *buf, size_t count)
{
	...
	shinkumara_powersave_bias_init();
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of shinkumara, waiting for disk IO is an
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, kshinkumara_wq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, kshinkumara_wq, &dbs_info->work,
			delay);
}
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		shinkumara_powersave_bias_init_cpu(cpu);
		/*
		...
		register_early_suspend(&shinkumara_power_suspend);
		pr_info("[shinkumara] shinkumara active\n");
		break;
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!dbs_enable)
			sysfs_remove_group(cpufreq_global_kobject,
					&dbs_attr_group);
		unregister_early_suspend(&shinkumara_power_suspend);
		pr_info("[shinkumara] shinkumara inactive\n");
		break;
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	kshinkumara_wq = create_workqueue("kshinkumara");
	if (!kshinkumara_wq) {
		printk(KERN_ERR "Creation of kshinkumara failed\n");
		return -EFAULT;
	}

	idle_notifier_register(&idle_notifier_block);

	err = cpufreq_register_governor(&cpufreq_gov_shinkumara);
	if (err)
		destroy_workqueue(kshinkumara_wq);
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_shinkumara);
	destroy_workqueue(kshinkumara_wq);
}

MODULE_AUTHOR("Khristian 2014 <khristian_chandra@yahoo.com>");
MODULE_AUTHOR("Shinkumara");
MODULE_DESCRIPTION("'cpufreq_shinkumara' - A dynamic cpufreq governor with a sleep profile based on ondemand for "
		"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SHINKUMARA
